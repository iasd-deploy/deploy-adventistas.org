...
class UjiCountdown extends Uji_Countdown {
    ...
    public function __construct() {
        ...
        //counts
        add_action( 'wp_footer', array( $this, 'uji_count_added' ) );
    }
    ...
    public function ujic_shortcode( $atts, $content = null ) {
        ...
        if ( $hide == "true" && $now_time > $unx_time ) {
            ...
            //save counter nr	
            static $ujic_arr = array();
            $ujic_arr = array_merge( $ujic_arr, array( $ujic_count ) );
            //update_option( 'ujicount', $ujic_arr );
            set_site_transient( 'ujicount', $ujic_arr, 60);
            ...
            $ujic_id = 'ujiCountdown' . $ujic_count;
            ...
            //enqueue
            if ( $ujic_style == 'modern') wp_enqueue_script( $this->plugin_slug . '-circles' );
            wp_enqueue_style( $this->plugin_slug . '-uji-countdown' );
            wp_enqueue_script( $this->plugin_slug . '-core' );
            wp_localize_script( $this->plugin_slug . '-init', 'ujiCount' . $ujic_count, array(
                ...
                'ujic_resp' => ( $this->ujic_get_option('ujic_resp') ) ? $this->ujic_get_option('ujic_resp')  : false,
            ...
            if ( $ujic_style == 'modern'){
                $mod_el = array('ujic_y' => 'uji_year', 'ujic_o' => 'uji_mont', 'ujic_w' => 'uji_week', 'ujic_d' => 'uji_days', 'ujic_h' => 'uji_hour', 'ujic_m' => 'uji_minu', 'ujic_s' => 'uji_secu');

                foreach( $mod_el as $mod => $_id ){
                    if( !empty(${$mod}) )
                        $modern  .= '<div class="ujicircle" id="'.$_id .'"></div>';
                }
            }
            ...
            {
                return strip_shortcodes( '<div class="ujic-hold' . $hclass . '"> <div class="ujiCountdown' . $classh . '" id="' . $ujic_id . '">' . $modern . '</div></div>' . $content );
            }
	    ...
	    $htmlCode = strip_shortcodes('<div class="ujic-hold' . $hclass . '"> <div id = "uji-wrapper" class = "ujicf"> <div class="ujicf ujiCountdown' . $classh . '" id="' . $ujic_id . '">'.$modern.'</div>'.$formHtmlCode.'</div></div>' . $content );
     ...
     * @since    2.0
     */
    public function uji_count_added() {
        $count = get_site_transient( 'ujicount' );
        wp_localize_script( $this->plugin_slug . '-init', 'ujiCounts', $count );
        wp_localize_script( $this->plugin_slug . '-init', 'ujiCount', array('ajaxUrl' => admin_url('admin-ajax.php')) );
    }
